...
class Application(tk.Frame):
    def __init__(self, window_width, window_height, master=None):
        ...
        # mouse double click event on the canvas keyboard
        self.canvas_keyboard.bind("<Double-Button-1>", self.mouse_left_double_click)
    ...
    def select_word_candidate(self, event):
        ...
        # Solution 4: 
        if btn.cget('text') == 'cmd S': # Selects 'cmd S' label in the middle frame will activate the command Save
            tkinter.messagebox.showinfo(title="Message box", message="Saved")
        elif btn.cget('text') == 'cmd C': # Selects 'cmd C' label in the middle frame will activate the command Copy
            tkinter.messagebox.showinfo(title="Message box", message="Copied")
        elif btn.cget('text') == '': # Selects an empty label will be noticed to reselect
            tkinter.messagebox.showinfo(title="Message box", message="Not a Command/Text")
        else:
    ...
    def mouse_left_button_release(self, event):
        ...
        if len(result) > 0:
            ...
            # Check whether the input contain words could active command inputs (Save, Copy)

            check_w = result[0][1]
            if check_w == 'save': # command input 'Save'
                self.label_word_candidates[0].config(text=result[0][1])
                self.label_word_candidates[1].config(text='cmd S')
            elif check_w == 'copy': # command input 'Copy'
                self.label_word_candidates[0].config(text=result[0][1])
                self.label_word_candidates[1].config(text='cmd C')
            else: # No command input, insert text
