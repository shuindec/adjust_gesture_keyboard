...
class Application(tk.Frame):
    def __init__(self, window_width, window_height, master=None):
        ...
        # Solution 1 - right mouse events on the keyboard       
    ...
    def mouse_right_button_release(self, event):
        ...
        if len(result) > 0:
            ...
            if key == '<--':  # remove the final character from the text
                ...
                tkinter.messagebox.showinfo(title="Message box", message="Save")
            elif key.lower() == 'c':
                tkinter.messagebox.showinfo(title="Message box", message="Copy")
            else:
                tkinter.messagebox.showinfo(title="Message box", message="Not a command")
